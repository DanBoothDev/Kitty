...
int main(int argc, char *argv[])
{
    ...
    // find where the file begins by checking if it begins with - signifying an arguement
    ...
    {
        // begin function with supplied filename which is located at fileIdx in the argv array
...
void readFile(char *file)
{
    ...
    if (inputFile == NULL)
    {
        // file doesn't exist or cannot be opened for some reason - print error and immediately terminate execution
        errnum = errno;
    ...
    while (1) 
    {
        bool beginning = false; // variable to print number BEFORE any input is read
        if (nflag || bflag)
        {
            beginning = true;
        }
        ...
        int count = 1; // variable that counts the amount of blank lines - used for -s command
        ...
        while (EOF != (input = fgetc(inputFile)))
        {
            ...
            {
                // set new line count to 0 as there is a character and not a new line to squeeze together
                count = 0;
            ...
            if (count <= 2)
            {
                ...
                if (vflag || tflag || eflag)
                {
                    if ((input < 33 && input >= 1) && (input != '\n'))
                    {
                        if (tflag && input == '\t')
                        {
                            input = input + 64;   /* forcing the special character to be a printable ascii character by casting it 64 places to be in the uppercase range */
                            printf("^%c", input); // print a hat then the assocated ascii character for the key pressed
                        }
                        else if (input != '\t')
                        {
                            input = input + 64;   /* forcing the special character to be a printable ascii character by casting it 64 places to be in the uppercase range */
                        ...
                        {
                            printf("\t"); // if its a tab character tab it accross so it is formatted as required
                        }
                    }
                    else
                    {
                ...
                {
                    fputc(input, stdout); // Where ALL characters that are standardly read in are printed out!
                }
            }
            ...
            if (input == '\n')
            {
                if (sflag && nflag)
                {
                    beginning = true; // return to print number at beginning of line before the next character
                }
                else if (nflag)
                {
